...
loader.load(CFG.url, gltf => {
  ...
  LS.set(1, 'Ready');
  LS.hide();
  setMode(MODES.POINTER);          // reveals by gliding from the dollhouse view down into the room
}, ev => {
...
(async () => {
  ...
  // deep link: ?compare=1 opens straight into the side-by-side
  if (new URLSearchParams(location.search).get('compare') === '1') ensureCloud().then(() => setSplit(true));
})();
